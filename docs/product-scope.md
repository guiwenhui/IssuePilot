...
M1 已于 2026-08-14 通过产品验收：Next.js 表单可创建任务，FastAPI 使用 Pydantic 校验输入，Task Service 通过异步 SQLAlchemy 将任务写入 PostgreSQL，详情页每 3 秒非重叠轮询一次任务状态。当前唯一可创建状态为 `created`。

M1 的 URL 校验只确认输入是无凭据、带主机名的 HTTPS URL；它不访问网络，也不证明仓库存在、公开、安全或可克隆。这些检查仍严格属于 M2。M1 同样不包含 Worker、Redis、RQ、LangGraph、Checkpoint、仓库文件操作或测试执行。

Redis 与 RQ 不在 M1 引入。M2 的简单后台执行暴露出重启丢失、并发或排队需求后，再于 M6 前评估是否升级；该升级目前不是既成事实。
